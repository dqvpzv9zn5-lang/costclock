...
const TEMPLATES = [
  // ─── 1. ACCOUNTING — CLIENT ONBOARDING ───────────────────────────────────
  {
    id: "onboarding-accounting",
  ...
  // ─── 2. ACCOUNTING — TAX RETURN ──────────────────────────────────────────
  {
    id: "tax-return",
    name: "Tax Return Filing",
    industry: "Accounting",
    description: "End-to-end tax return from data collection to HMRC submission.",
    annualVolume: 200,
    steps: [
      ...
      { name: "Log documents received & review completeness", roleId: "admin", minutes: 15, friction: "medium", workType: "manual" },
      { name: "Request missing information", roleId: "senior", minutes: 15, friction: "high", workType: "manual" },
      { name: "Chase missing information", roleId: "admin", minutes: 20, friction: "high", workType: "waiting" },
      ...
      { name: "Chase client approval", roleId: "admin", minutes: 25, friction: "high", workType: "waiting" },
      { name: "File with HMRC", roleId: "senior", minutes: 15, friction: "low", workType: "manual" },
      { name: "Confirm filing & update deadline tracker", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
      { name: "Issue invoice to client", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
      { name: "Chase invoice payment", roleId: "admin", minutes: 20, friction: "high", workType: "waiting" },
    ],
  },
  // ─── 3. LEGAL — NEW MATTER OPENING ───────────────────────────────────────
  {
    id: "new-matter-legal",
    name: "New Matter Opening",
    industry: "Legal",
    description: "Opening a new legal matter from instruction to active case management.",
    annualVolume: 120,
    steps: [
  ...
  // ─── 4. RECRUITMENT — CANDIDATE PLACEMENT ────────────────────────────────
  {
    id: "placement-recruitment",
    name: "Candidate Placement",
    industry: "Recruitment",
    description: "From job brief through to placement confirmation and invoicing.",
    annualVolume: 150,
    steps: [
      { name: "Receive & log job brief from client", roleId: "admin", minutes: 15, friction: "low", workType: "manual" },
      { name: "Qualify brief & agree terms", roleId: "manager", minutes: 20, friction: "low", workType: "decision" },
      { name: "Post job across boards", roleId: "admin", minutes: 20, friction: "medium", workType: "manual" },
      { name: "Log candidates into ATS / CRM", roleId: "admin", minutes: 15, friction: "medium", workType: "manual" },
      ...
      { name: "Update ATS at each pipeline stage", roleId: "admin", minutes: 10, friction: "medium", workType: "manual" },
      { name: "Schedule interviews", roleId: "admin", minutes: 20, friction: "high", workType: "manual" },
      { name: "Send candidate status updates to client", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
      ...
      { name: "Compliance doc collection (DBS, RTW)", roleId: "admin", minutes: 25, friction: "high", workType: "manual" },
      ...
      { name: "Chase contract signature", roleId: "admin", minutes: 15, friction: "high", workType: "waiting" },
      { name: "Raise invoice to client", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
      { name: "Chase invoice payment", roleId: "admin", minutes: 20, friction: "high", workType: "waiting" },
    ],
  },
  // ─── 5. PROPERTY — TENANT ONBOARDING ─────────────────────────────────────
  {
    id: "tenant-onboarding",
    name: "Tenant Onboarding",
    industry: "Property Management",
    description: "New tenant from viewing through to move-in and system setup.",
    annualVolume: 150,
    steps: [
      ...
      { name: "Notify landlord of tenancy start", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
      { name: "Set up recurring rent collection", roleId: "admin", minutes: 15, friction: "medium", workType: "manual" },
      ...
      { name: "Key handover & welcome pack", roleId: "admin", minutes: 20, friction: "low", workType: "manual" },
      { name: "Schedule first inspection reminder", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
    ],
  },
  // ─── 6. IFA — ANNUAL CLIENT REVIEW ───────────────────────────────────────
  {
    id: "client-review-ifa",
    name: "Annual Client Review",
    industry: "Financial Advice",
    description: "Ongoing client review cycle from preparation to implementation.",
    annualVolume: 150,
    steps: [
      ...
      { name: "Chase client to confirm meeting", roleId: "admin", minutes: 15, friction: "high", workType: "waiting" },
      ...
      { name: "Write up meeting notes & actions", roleId: "senior", minutes: 25, friction: "medium", workType: "manual" },
      ...
      { name: "Update client risk profile if changed", roleId: "senior", minutes: 15, friction: "low", workType: "manual" },
      { name: "Schedule next annual review", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
    ],
  },
  // ─── 7. HEALTHCARE — PATIENT INTAKE & ASSESSMENT ─────────────────────────
  {
    id: "patient-intake-healthcare",
    name: "Patient Intake & Assessment",
    industry: "Healthcare",
    description: "Private clinic intake from referral to completed assessment and billing.",
    annualVolume: 300,
    steps: [
      ...
      { name: "Write up assessment report", roleId: "partner", minutes: 60, friction: "medium", workType: "decision" },
      ...
      { name: "Send follow-up & next steps to patient", roleId: "admin", minutes: 10, friction: "low", workType: "manual" },
    ],
  },
  // ─── 8. CONSTRUCTION — PROJECT DELIVERY ──────────────────────────────────
  {
    id: "project-delivery-construction",
    name: "Project Delivery",
    industry: "Construction",
    description: "From tender award to practical completion. Tracks the admin, compliance, and payment chasing that eats into every project margin.",
    annualVolume: 24,
    steps: [
      ...
      { name: "Chase missing compliance certificates", roleId: "admin", minutes: 25, friction: "high", workType: "waiting" },
      ...
      { name: "Distribute meeting minutes & actions", roleId: "admin", minutes: 15, friction: "low", workType: "manual" },
  ...
  // ─── 9. SCRATCH ───────────────────────────────────────────────────────────
  {
    ...
    industry: "Any",
    description: "Build your own process map from a blank canvas.",
    annualVolume: 50,
    steps: [],
  },
];
...
function Select({ value, onChange, options, style }) {
  ...
  selectStyle.background = `${bgColor} url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='${arrowColor}' stroke-width='3' stroke-linecap='round' stroke-linejoin='round'%3e%3cpolyline points='6 9 12 15 18 9'%3e%3c/polyline%3e%3c/svg%3e") no-repeat right 12px center / 12px`;
  // Re-assert backgroundSize: the `background` shorthand resets it in the browser
  selectStyle.backgroundSize = "12px";
    ...
    <div style={{maxWidth:1080,margin:"0 auto",padding:"108px 40px 80px",position:"relative",zIndex:1}} className="page-pad">
      ...
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:16,marginBottom:20,...anim(0.4)}}>
          ...
          <Card style={{background:"#d4ede2",border:"1px solid #a8dcc0"}}><div style={{fontSize:"0.7rem",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.08em",color:"#1b4332",marginBottom:8}}>Automation opportunities</div><div style={{fontFamily:"'Outfit',sans-serif",fontSize:"1.8rem",fontWeight:700,color:"#2d6a4f"}}>{saveableSteps.length}</div><div style={{fontSize:"0.8rem",color:"#2d6a4f",marginTop:4}}>saving {saveableMins}m per run</div></Card>
        ...
        {screen!=="welcome"&&(
          <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,background:"rgba(250,249,247,0.97)",backdropFilter:"blur(12px)",borderBottom:"1px solid #e5e2dc"}}>
            ...
            <div style={{maxWidth:1080,width:"100%",margin:"0 auto",padding:"0 20px",height:48,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <button onClick={reset} style={{background:"none",border:"none",cursor:"pointer",padding:0,opacity:navMounted?1:0,transition:"opacity 0.4s ease",marginLeft:20,marginTop:2}}>
